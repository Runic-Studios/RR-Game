 ...
 */
class ArmorEquipListener @Inject constructor(
    ...
    fun inventoryClick(e: InventoryClickEvent) {
        ...
        if (shift) {
            ...
            if (newArmorType != null) {
                ...
                if (newArmorType == ArmorType.HELMET
                    && (equipping == isAirOrNull(e.whoClicked.inventory.helmet)) || newArmorType == ArmorType.CHESTPLATE
                    && (equipping == isAirOrNull(e.whoClicked.inventory.chestplate)) || newArmorType == ArmorType.LEGGINGS
                    && (equipping == isAirOrNull(e.whoClicked.inventory.leggings)) || newArmorType == ArmorType.BOOTS
            ...
            if (numberKey) {
                ...
                if (isAirOrNull(e.cursor) && !isAirOrNull(e.currentItem)) { // un-equip with no new item going into the slot.
                    newArmorType = ArmorType.matchType(e.currentItem)
                }
    ...
    fun playerInteractEvent(event: PlayerInteractEvent) {
        ...
        if (event.useInteractedBlock() != Event.Result.DENY) {
            if (event.clickedBlock != null && event.action == Action.RIGHT_CLICK_BLOCK && !player.isSneaking) {
    ...
    companion object {
        private val BLOCKED_MATERIALS = HashSet<Material>().apply {
