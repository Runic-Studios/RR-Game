...
class ArmorEquipListener @Inject constructor(private val plugin: Plugin) : Listener {
    ...
    suspend fun inventoryClick(event: InventoryClickEvent) {
        ...
        if (shift) {
            ...
            if (newArmorType != null) {
                val equipping = event.rawSlot != newArmorType.slot
                if (
            ...
            var newArmorPiece = event.cursor
            var oldArmorPiece = event.currentItem
            if (numberKey) {
                if (
                    ...
                    if (!isAirOrNull(hotbarItem)) { // Equipping
                        ...
                        oldArmorPiece = event.clickedInventory!!.getItem(event.slot)
                    } else { // un-equipping
                        newArmorType =
                            ArmorType.matchType(
                ...
                if (
                    isAirOrNull(event.cursor) && !isAirOrNull(event.currentItem)
    ...
    @EventHandler(priority = EventPriority.HIGHEST)
    suspend fun inventoryDrag(event: InventoryDragEvent) {
