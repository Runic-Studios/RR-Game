...
sealed class GameItemTemplate(
    ...
    protected open fun buildItemData(count: Int): ItemData.Builder {
        val builder = ItemData.newBuilder()
            .setTemplateID(id)
            .setCount(count)
        return builder
    }
    ...
    protected fun LinkedHashMap<StatType, StatRange>.toRolledStats(): List<ItemData.RolledStat> {
        ...
        for ((statType, _) in this) {
            val stat = ItemData.RolledStat.newBuilder()
                .setType(statType)
                .setRollPercentage(ThreadLocalRandom.current().nextDouble())  // Roll random value
                .build()
            stats.add(stat)
        }
        return stats
    ...
    protected fun LinkedHashMap<String, Int>.toPerks(): List<ItemData.Perk> {
        ...
        for ((perkID, stacks) in this) {
            val perk = ItemData.Perk.newBuilder()
                .setPerkID(perkID)
                .setStacks(stacks)
                .build()
            perks.add(perk)
    ...
    protected fun LinkedHashMap<GameItemClickTrigger.Type, String>.toTriggers(): List<GameItemClickTrigger> {
