...
/**
 * NOTE: This class relies on legacy serialization because of a few issues.
 * If we were to use Component#replaceText in adventure:
 ...
 *
 * Instead, we use an efficient algorithm that is capable of parsing the legacy strings
 * by looking at angle brackets.
 */
class DynamicItemManager @Inject constructor(
    plugin: Plugin,
    ...
    override fun onPacketSending(event: PacketEvent) {
        val character = dataAPI.getCharacter(event.player.uniqueId) ?: return
    ...
    fun processItem(viewer: GameCharacter, itemStack: ItemStack) { // Try and be as efficient as we can!
        ...
        if (displayNameComponent != null) {
            val displayName = LegacyComponentSerializer.legacySection().serialize(displayNameComponent)
            val replacement = generateReplacement(displayName, viewer, gameItem, itemStack)
            if (replacement != null) {
                val newComponent = LegacyComponentSerializer.legacySection().deserialize(replacement)
                // So this is a really really really annoying workaround because the adventure API is sometimes
                // really hard to work with TODO fix
                val noItalics = Component.text().decoration(TextDecoration.ITALIC, false).append(newComponent).build()
        ...
        if (meta.hasLore()) {
            ...
            for (component in lore) {
                ...
                if (replacement != null) {
                    changed = true
                    val newComponent: Component = LegacyComponentSerializer.legacySection().deserialize(replacement)
                    // So this is a really really really annoying workaround because the adventure API is sometimes
                    // really hard to work with TODO fix
                    val noItalics: Component =
                        Component.text().decoration(TextDecoration.ITALIC, false).append(newComponent).build()
    ...
    private fun generateReplacement(
        ...
        while (!results.isEmpty()) {
            ...
            if (placeholder != null) {
                val replacement = placeholder.generateReplacement(viewer, gameItem, itemStack) ?: continue
    ...
    @JvmRecord
    private data class BracketedTextInfo(val beginIndex: Int, val endIndex: Int, val bracketedText: String)
    ...
    companion object {
        ...
        private fun replaceStringSegment(
            ...
            require(!(beginIndex < 0 || endIndex > original.length || beginIndex > endIndex)) { "Invalid indices" }
