...
object GemStatUtil {
    ...
    private val GEM_STAT_OPTIONS: MutableMap<Int, List<Pair<Int, IntArray>>> =
        HashMap()
     ...
     * Tier should be a number between 1 and 5, **0 isn't a tier**.
     *
    ...
    fun generateGemBonuses(tier: Int, mainStat: StatType): List<ItemData.StaticStat> {
        require(!(tier < 0 || tier > GEM_STAT_OPTIONS.size)) { "Tier $tier does not exist for gem bonuses." }
        ...
        val mainStaticStat = ItemData.StaticStat.newBuilder()
            .setType(mainStat)
        ...
        for (subStatBonus in selectedOption.second) {
            ...
            val staticStat = ItemData.StaticStat.newBuilder()
