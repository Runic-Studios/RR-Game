...
abstract class DynamicItemPerkTextPlaceholder
protected constructor(
    placeholder: String,
    private val characterEquipmentCacheRegistry: CharacterEquipmentCacheRegistry,
) : DynamicItemTextPlaceholder(placeholder) {
    /** Gets the slot in which an item is currently equipped Returns null if not equipped */
    protected fun getEquippedSlot(
        player: Player,
        gameItem: GameItem,
        itemStack: ItemStack,
    ): EquippedSlot? {
        ...
        if (ArmorEquipEvent.ArmorType.matchType(itemStack) != null) {
            ...
            if (
                helmet != null &&
                    itemHelmet != null &&
                    helmet.template.id == template.id &&
            ...
            if (
                chestplate != null &&
                    itemChestplate != null &&
                    chestplate.template.id == template.id &&
            ...
            if (
                leggings != null &&
                    itemLeggings != null &&
                    leggings.template.id == template.id &&
        ...
        if (offhand != null && offhand.template.id == template.id && itemOffhand == itemStack)
            return EquippedSlot.OFFHAND

        val weapon = cache.getWeapon()
        val itemWeapon = player.inventory.itemInMainHand
        if (weapon != null && weapon.template.id == template.id && itemWeapon == itemStack)
