...
class GameCharacter internal constructor(plugin: Plugin, gameSession: GameSession) :
    ...
    /**
     * ALL CALLS to withSyncCharacterData MUST be on the Minecraft game thread!
     * (Hence the sync in its name).
     *
    ...
    fun <T> withSyncCharacterData(action: UserCharacterData.() -> T): T {
        if (!Bukkit.isPrimaryThread()) throw IllegalStateException("Cannot modify game player data on async thread!")
        return gameSession.characterData!!.action()
    }

    /**
     * withCharacterData can be called on any thread, but will suspend the context
     * and swap to the main thread if run on an async worker.
     *
     * Computation can be offloaded to worker threads, but final handling of data must occur on the
     * main thread.
     */
